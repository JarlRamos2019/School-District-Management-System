...
if (isset($_POST["delete"])) {
    ...
    if (!empty($_POST["delete_id"])) {
        ...
        $verification = $db->prepare("select * from Teacher where TeacherID = ?");
        $verification->bindParam(1, $_POST["delete_id"], PDO::PARAM_INT);
        $verification->execute();
        $res = $verification->fetchAll(PDO::FETCH_ASSOC);
        if (empty($res)) echo "Sorry, no teacher with the provided ID was found.";
        else {
            $query = $db->prepare("delete from Teacher where TeacherID = ?");
    ...
    } else echo "Unable to delete: missing Teacher ID<br>";
    unset($_POST["delete"]);
}
?>
<html>
...
<body>
    <h1><?= $PROJECT_NAME ?></h1>
    <?php include("nav.php"); ?>
    <h2>Delete Teacher Information</h2>
...
$delete_form->add_input("Teacher ID: ", array(
    "type" => "number"
), "delete_id");
